...
* **Emergency Injection:** Authorized supervisors can inject new image URLs live during a briefing using an `ox_lib` input dialog.
...
Ensure your file structure perfectly matches the following:
...
├── server.lua
└── fxmanifest.lua
Add the resource to your server.cfg. Start order is critical.
```
ensure ox_lib
ensure ox_target
ensure es_extended
...
Usage Guide
Start Presentation: Target the configured laptop and select "Start Presentation" to turn on the screen and load Slide 1.

Navigation: Target the laptop to advance to the "Next Slide" or return to the "Previous Slide".

Emergency Add: If a new piece of evidence is discovered mid-briefing, select "Emergency Add URL" to append a new direct image link to the end of the live presentation.

Clear: Select "Turn Projector Off" to instantly terminate the presentation and clear the screen for all players.
